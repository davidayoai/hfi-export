#!/usr/bin/env python3
"""
hfi-export.py — Collect on-disk claude-hfi metadata into a single .tar.gz.

Intended for labelers to run on their own laptops so we can recover session data
after the fact. Zero dependencies, stdlib only. macOS + Linux (the only platforms
claude-hfi ships on).

What it collects
----------------
1. <tmpdir>/claude-hfi/     — IPC session dirs (prompt-N.json, result-N-{A,B}.json,
                              feedback/submission/experiment-config JSONs, debug.txt).
                              These are the ephemeral files OS temp cleanup wipes.
2. ~/.claude-hfi/           — Config dir: projects/ (trajectory transcript JSONLs),
                              debug/, history.jsonl, bundle-cache/, backups/, todos/.
                              Excludes plugins/ (marketplace clone, large & irrelevant).
3. hfi/* git branch listing — Scanned from ~/.cache/claude-hfi/ worktree pointers so
                              the HFI-session-ID → branch mapping is recorded, without
                              shipping the actual worktree code.

Output
------
~/Desktop/claude-hfi-export-<hostname>-<YYYYMMDD-HHMMSS>.tar.gz
(falls back to ~/ if Desktop doesn't exist)

Archive layout:
    claude-hfi-export-<hostname>-<ts>/
        manifest.json       — hostname, timestamps, per-source stats, warnings
        tmpdir/             — everything from <tmpdir>/claude-hfi/
        config-dir/         — everything from ~/.claude-hfi/ minus plugins/
        git-branches.txt    — hfi/* branches + worktree list for each base repo found

Usage
-----
    python3 hfi-export.py
    python3 hfi-export.py --out /path/to/output.tar.gz
    python3 hfi-export.py --redact-auth     # drop .claude.json* (contains Auth0 token)

Note on auth: ~/.claude-hfi/.claude.json holds the Auth0 bearer token. It's
included by default so the export is truly complete, but pass --redact-auth if
you're forwarding the tarball to someone who shouldn't hold the labeler's token.
"""
